...
const reversePeopleName = people.map(name => {
  const [last, first] = name.split(', ');
  return `${first} ${last}`;
});

console.log(reversePeopleName);
...
const nineteenOlder = devs.some(dev => (currentYear - dev.year) >= 19);
...
const allNineteenOlder = devs.every(dev => (currentYear - dev.year) >= 19);

console.log(allNineteenOlder);
